export default function Styles() {
  return (
    <style global jsx>{`
      :root {
        --bg-start: #000000;
        --bg-end: #000000;
        --accent: #8c75cc;
        --accent-2: #3e4d84;
        --accent-hover: #7c69b7;
        --accent-2-hover: #384476;
      }

      html,
      body,
      #root {
        height: 100%;
        overflow-x: hidden;
      }

      body {
        margin: 0;
        background: radial-gradient(
            1200px 800px at 10% 20%,
            rgba(141, 118, 204, 0.12),
            transparent 60%
          ),
          radial-gradient(
            900px 700px at 80% 60%,
            rgba(62, 77, 132, 0.12),
            transparent 60%
          ),
          linear-gradient(180deg, var(--bg-start), var(--bg-end)) !important;
        color: white !important;
        font-family: "Overpass", ui-sans-serif, system-ui, -apple-system, Segoe UI,
          Roboto, Helvetica, Arial, "Apple Color Emoji", "Segoe UI Emoji";
      }

      .title {
        font-family: "OverpassBold", ui-sans-serif, system-ui, -apple-system, Segoe UI,
          Roboto, Helvetica, Arial, "Apple Color Emoji", "Segoe UI Emoji" !important;
      }

      * {
          scrollbar-width: thin;
          scrollbar-color: #0a0a0c #070707;
      }

      *::-webkit-scrollbar,
      html::-webkit-scrollbar,
      body::-webkit-scrollbar,
      .main-scroll-container::-webkit-scrollbar {
          width: 8px !important;
          background: #070707 !important;
      }

      *::-webkit-scrollbar-thumb,
      html::-webkit-scrollbar-thumb,
      body::-webkit-scrollbar-thumb,
      .main-scroll-container::-webkit-scrollbar-thumb {
          background: #0a0a0c !important;
          border-radius: 8px !important;
      }

      *::-webkit-scrollbar-track,
      html::-webkit-scrollbar-track,
      body::-webkit-scrollbar-track,
      .main-scroll-container::-webkit-scrollbar-track {
          background: #070707 !important;
      }


      button {
        font-family: "OverpassBold";
      }

      @keyframes sway {
        0% {
          transform: translate(0px, 0px) rotate(0deg);
        }
        25% {
          transform: translate(3px, 2px) rotate(1deg);
        }
        75% {
          transform: translate(-3px, 1px) rotate(-1deg);
        }
        100% {
          transform: translate(0px, 0px) rotate(0deg);
        }
      }

      .grid-item {
        grid-column: span 1;
        transition: grid-column 0.5s cubic-bezier(0.4, 0, 0.2, 1),
          transform 0.5s cubic-bezier(0.4, 0, 0.2, 1),
          max-height 0.5s cubic-bezier(0.4, 0, 0.2, 1),
          opacity 0.5s cubic-bezier(0.4, 0, 0.2, 1);
      }

      .grid-item.expanded {
        grid-column: 1 / -1;
      }

      .expanded-content {
        max-height: 0;
        opacity: 0;
        overflow: hidden;
        transition: max-height 0.5s cubic-bezier(0.4, 0, 0.2, 1),
          opacity 0.5s cubic-bezier(0.4, 0, 0.2, 1),
          transform 0.5s cubic-bezier(0.4, 0, 0.2, 1);
      }

      .expanded .expanded-content {
        max-height: 500px;
        opacity: 1;
        transform: translateY(0);
      }

      @layer utilities {
        .bloom {
          filter: blur(40px);
          opacity: 0.6;
        }
        .bloom-strong {
          filter: blur(70px);
          opacity: 0.5;
        }
        .text-glow {
          text-shadow: 0 0 12px rgba(141, 118, 204, 0.5),
            0 0 22px rgba(62, 77, 132, 0.35);
        }
        .ring-glow {
          box-shadow: 0 0 0 1px rgba(148, 163, 184, 0.15),
            0 0 24px rgba(141, 118, 204, 0.35), inset 0 0 12px rgba(62, 77, 132, 0.2);
        }

        .glint {
          position: relative;
          overflow: hidden;
        }
        .glint::before {
          content: "";
          position: absolute;
          top: 0;
          left: 0;
          right: 0;
          height: 18%;
          background: linear-gradient(
            to bottom,
            rgba(255, 255, 255, 0.22),
            rgba(255, 255, 255, 0.06) 20%,
            transparent
          );
          pointer-events: none;
          border-top-left-radius: inherit;
          border-top-right-radius: inherit;
        }
      }

      .glassish {
        background: rgba(15, 23, 42, 0.45) !important;
        backdrop-filter: blur(16px) saturate(140%) !important;
        -webkit-backdrop-filter: blur(16px) saturate(140%) !important;
        border: 1px solid rgba(148, 163, 184, 0.15) !important;
      }

      .carousel {
        transform: none;
        max-width: 100%;
        box-sizing: border-box;
        width: 100%;
        overflow: visible;
      }

      .carousel-container {
        transform: scale(0.9);
        padding-right: 4rem;
      }

      @media (min-width: 768px) {
        .carousel {
          transform: perspective(1000px) rotateY(-6deg);
        }
      }

      @media (max-width: 1211px) {
        .carousel-container {
          position: relative !important;
          right: auto !important;
          top: auto !important;
          transform: none !important;
          width: 100% !important;
          max-width: 100% !important;
          display: none !important;
          margin-top: 2rem;
        }

        .left-content {
          padding-left: 3rem;
          max-width: 50rem;
        }
      }

      @media screen and (max-width: 950px) {
        .bg-\[\#1a1a1f\] {
          background-color: transparent !important;
        }
      }

      @media (max-width: 767px) {
        .mobile-container {
          max-width: 130vw;
          overflow-x: hidden;
          padding-top: 4rem;
        }

        .by {
          display: none;
        }

        .mobile-full-width {
          width: 100%;
          max-width: 100%;
        }

        .left-content {
          padding-left: unset;
          max-width: unset;
        }
        .carousel-container,
        .carousel {
          display: block !important;
          margin-left: 1rem !important;
          margin-bottom: 0.2rem;
        }
        #descrip {
          margin-left: 2rem;
        }
      }

      img {
        border-radius: 25px;
      }

      .disabledAPK {
        filter: grayscale(1);
        cursor: not-allowed !important;
      }

      .forceSlightRound {
        border-radius: 5px !important;
      }

      .glassish {
        background: rgba(15, 23, 42, 0.45); /* Sfondo semi-trasparente */
        backdrop-filter: blur(16px) saturate(140%); /* Effetto di sfocatura dello sfondo */
        -webkit-backdrop-filter: blur(16px) saturate(140%); /* Prefisso per WebKit */
        border: 1px solid rgba(148, 163, 184, 0.15); /* Bordo sottile e leggero */
        box-shadow: 0 0 0 2px rgba(148, 163, 184, 0.15),
            0 0 24px rgba(141, 118, 204, 0.35), inset 0 0 12px rgba(62, 77, 132, 0.2);
      }

      .glassish-input {
          background: rgba(30, 40, 60, 0.7); /* Meno trasparente per leggibilità */
          backdrop-filter: blur(4px); /* Blur minore */
          -webkit-backdrop-filter: blur(4px);
          border: 1px solid rgba(148, 163, 184, 0.1);
      }

      .bloom-strong1 {
          filter: blur(70px);
          opacity: 0.3;
      }
      
      .mintlify-sidebar {
        display: block !important;
        background-color: transparent !important;
      }

      html[data-theme='light'] .mintlify-sidebar a {
        color: #000000 !important;
        opacity: 0.9;
      }

      html[data-theme='light'] .mintlify-sidebar a:hover {
        color: #8c75cc !important;
        opacity: 1;
      }

      html[data-theme='light'] .mintlify-sidebar a.active {
        color: #baace0 !important;
        font-weight: 600;
      }

      html[data-theme='dark'] .mintlify-sidebar a {
        color: #aaaaaa !important;
      }

      html[data-theme='dark'] .mintlify-sidebar a:hover {
        color: #ffffff !important;
      }

      html[data-theme='dark'] .mintlify-sidebar a.active,
      html[data-theme='dark'] .mintlify-sidebar a[aria-current='page'] {
        color: #4da6ff !important;
      }
    `}</style>
  );
}